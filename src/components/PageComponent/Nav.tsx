"use client"
import { abbyImage } from "@/data/abbyData/data"
import { useSwitch } from "../Context/SwitchContext"
import { profileImage, profileShortName } from "@/data/ProfileData/data"
  ...
  return (
    <div className="fixed bottom-6 left-0 right-0 flex justify-center items-center">
      <div className="relative bg-mainBlack border border-zinc-700 px-3 py-2 rounded-xl flex items-center gap-3">
        ...
        >
          <Image
            src={isSwitchOn ? profileImage : abbyImage}
            alt="Profile Picture"
            className="rounded-md"
            width={35}
            height={35}
            // title={isSwitchOn ? profileShortName : abbyName}
          />
        </div>
        {isArrowVisible && (
